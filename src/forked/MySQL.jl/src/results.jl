...
"""
@compat mysql_interpret_field(strval::String, ::Type{Cuchar}) = UInt8(strval[1])
...
function mysql_get_field_types(mysqlfields::Vector{MYSQL_FIELD})
    nfields = length(mysqlfields)
    mysqlfield_types = Vector{MYSQL_TYPE}(nfields)
    for i = 1:nfields
...
function mysql_get_row_as_vector(result, jtypes, isnullable)
    retvec = Vector{Any}(length(jtypes))
...
function mysql_get_nullable(meta::Vector{MYSQL_FIELD})
    isnullable = Vector{Bool}(length(meta))
    for i = 1:length(meta)
...
function mysql_get_result_as_array(result)
    ...
    retarr = Vector{Vector{Any}}(nrows)
    for i = 1:nrows
        retarr[i] = Vector{Any}(meta.nfields)
...
function mysql_bind_array(meta::MySQLMetadata)
    bindarr = Vector{MYSQL_BIND}(meta.nfields)
